...
namespace XTP {
	namespace API {
		...
		public:
			...
			virtual void OnOrderEvent(XTPOrderInfo_ *order_info, XTPRI *error_info) {};
			...
			virtual void OnTradeEvent(XTPTradeReport_ *trade_info) {};
			...
			virtual void OnCancelOrderError(XTPOrderCancelInfo_ *cancel_info, XTPRI *error_info) {};
			...
			virtual void OnQueryOrder(XTPOrderInfo_ *order_info, XTPRI *error_info, int request_id, bool is_last) {};
			...
			///@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
			///@remark 由于支持分时段查询，一个查询请求可能对应多个响应，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
			virtual void OnQueryTrade(XTPTradeReport_ *trade_info, XTPRI *error_info, int request_id, bool is_last) {};
			...
			virtual void OnQueryPosition(XTPQueryStkPositionRsp_ *position, XTPRI *error_info, int request_id, bool is_last) {};
			...
			virtual void OnQueryAsset(XTPQueryAssetRsp_ *asset, XTPRI *error_info, int request_id, bool is_last) {};
...
namespace XTP {
	namespace API {
		...
		public:
			...
			static TraderApi *CreateTraderApi(uint8_t client_id, const char *save_file_path, XTP_LOG_LEVEL_ log_level = XTP_LOG_LEVEL_DEBUG);
			...
			virtual void SubscribePublicTopic(XTP_TE_RESUME_TYPE_ resume_type) = 0;
			...
			virtual uint64_t Login(const char* ip, int port, const char* user, const char* password, XTP_PROTOCOL_TYPE_ sock_type) = 0;
			...
			virtual uint64_t InsertOrder(XTPOrderInsertInfo_ *order, uint64_t session_id) = 0;
			...
			virtual int QueryOrders(const XTPQueryOrderReq_ *query_param, uint64_t session_id, int request_id) = 0;
			...
			virtual int QueryTrades(XTPQueryTraderReq_ *query_param, uint64_t session_id, int request_id) = 0;
